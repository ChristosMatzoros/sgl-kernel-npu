...
uint32_t maxTileWidthForRing(uint32_t ringSize, bool useA5)
{
    if (useA5) {
        switch (ringSize) {
                                       ...
                                       bool activation_mode, int64_t pad_slot_id)
{
    ...
    // A5 (dav-c310) has a larger UB, so it uses wider channel tiles (fewer tiles per
    // sequence). Pick the tile-width table from the running SoC so it matches the arch
    // the kernel was compiled for (the host has no compile-time device-arch macro).
    const bool useA5 = (plat->GetSocVersion() == platform_ascendc::SocVersion::ASCEND950);
    const uint32_t maxChannelsPerTile = maxTileWidthForRing(ringSize, useA5);  // UB-bound tile width
